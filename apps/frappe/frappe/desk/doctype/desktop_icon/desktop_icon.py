	...
	def check_app_permission(self, app_name):
		for a in frappe.get_installed_apps():
			if frappe.get_hooks(app_name=a)["app_title"][0] == app_name or self.app == a:
				try:
					add_to_apps_screen = frappe.get_hooks(app_name=a).get("add_to_apps_screen", [])
					if add_to_apps_screen and isinstance(add_to_apps_screen[0], dict):
						permission_method = add_to_apps_screen[0].get("has_permission", None)
					else:
